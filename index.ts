...
const INT_STEP = 1 / 1024;
const DER_STEP = 1 / 1024 / 256;

function ___integrate(fPosDef: RealFunction, min: number, max: number): number {
    let res = 0;
    for (; min + INT_STEP <= max; min += INT_STEP) res += INT_STEP * (fPosDef(min) + fPosDef(min + INT_STEP)) / 2;
...
function integrate(f: RealFunction, pow: number = 1, a: number = 0): RealFunction {
    if (pow < 0) {
        ...
        const int = integrate(f, k + pow);
        return _derivate(int, k);
    }
    if (pow === 0) return f;
